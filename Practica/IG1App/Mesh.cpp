...
// Crea un pol�gono regular de n lados y radio r
Mesh*
...
// Crea un tri�ngulo equil�tero de colores de lado r partiendo de la maya de un pol�gono regular de 3 lados
Mesh*
...
// Crea un rect�ngulo de lados w y h
Mesh*
Mesh::generateRectangle(GLdouble w, GLdouble h) {
	...
	// En este orden devuelve el rect�ngulo relleno?
	//mesh->vVertices.emplace_back(x, -y, z);
	//mesh->vVertices.emplace_back(-x, -y, z);
	//mesh->vVertices.emplace_back(x, y, z);
	//mesh->vVertices.emplace_back(-x, y, z);
...
// Crea un rect�ngulo de lados w y h con colores
Mesh*
...
Mesh::generateCube(GLdouble length) {
	...
	// PRUEBA MENOS V�RTICES, A OPTIMIZAR EN FUTURO
	//mesh->vVertices.emplace_back(l, l, l);		//1
	//mesh->vVertices.emplace_back(-l, l, l);	//2
	//mesh->vVertices.emplace_back(-l, l, -l);	//3
	//mesh->vVertices.emplace_back(-l, -l, -l);	//4
...
// Crea un cubo a partir del m�todo GenerateCube y le asigna colores a cada cara
Mesh* 
Mesh::generateRGBCubeTriangles(GLdouble length) {
	Mesh* mesh = generateCube(length);
	// Reservar los v�rtices a los que les vas a asignar color
	mesh->vColors.reserve(36);

	// Definir los colores de cada cara y los n�meros de v�rtices en los que se cambia de cara
	int r = 0, g = 0, b = 0;
	int minusYI = 0, minusYF = 5, minusXI = 6, minusXF = 11, minusZI = 12, minusZF = 17,
		YI = 18, YF = 23, XI = 24, XF = 29, ZI = 30, ZF = 35;

	// Asignar colores a cada v�rtice
	for (int i = 0; i < 36; ++i) {
